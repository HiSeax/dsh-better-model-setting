...
interface BetterModelSetting {
  // disabledProviders 不禁用状态真源，禁用列表来自 status.disabledProviders（DB 快照）
  builtinDisabled: string[]
  providerOrder: string[]
  modelEfforts: Record<string, Record<string, ModelEffortSetting | string>>
  providerRetryOverrides: Record<string, {
    maxRetries?: number
    mode?: string
    retryableCodes?: string[]
    backoff?: any
  }>
  officialAdded?: boolean
}
interface ModelEffortSetting { tiers?: string[]; selected?: string }
interface ProviderProfile {
  apiKeyEnv?: string; displayName?: string; api?: string; baseURL?: string
...
interface StatusPayload {
  ...
  enabledProviders: Record<string, ProviderProfile>
  disabledProviders: Record<string, ProviderProfile>
...
async function postCommand(body: object): Promise<StatusPayload> {
...
// ---------------------------------------------------------------------------
function normalizeEffortEntry(value: any): ModelEffortSetting {
  ...
  if (value && typeof value === 'object') return {
    ...
    tiers: Array.isArray(value.tiers) ? value.tiers.filter((t: any) => typeof t === 'string') : undefined,
  }
  return {}
}
function normalizeSetting(raw: any): BetterModelSetting {
  if (!raw || typeof raw !== 'object') return { builtinDisabled: [], providerOrder: [], modelEfforts: {}, providerRetryOverrides: {} }
  const modelEfforts: Record<string, Record<string, ModelEffortSetting | string>> = {}
  ...
  const strings = (v: any) => Array.isArray(v) ? v.filter((x: any) => typeof x === 'string') : []
  return {
...
function toModelRow(model: any): ModelRow | null {
...
function toRow(id: string, profile: ProviderProfile, active: boolean, kind: 'config' | 'builtin' = 'config', cred?: string): ProviderRow {
  return {
...
function mergeRows(status: StatusPayload): ProviderRow[] {
  ...
  for (const id of Object.keys(status.enabledProviders || {})) {
    ...
    if (p && typeof p === 'object') { rows.push(toRow(id, p, true, 'config', creds[id])); seen.add(id) }
  }
  for (const id of Object.keys(status.disabledProviders || {})) {
    const p = status.disabledProviders[id]
    if (p && typeof p === 'object') { rows.push(toRow(id, p, false, 'config', creds[id])); seen.add(id) }
  }
  const builtinDisabled = new Set(status.builtinDisabled || [])
  for (const id of Object.keys(status.builtinProviders || {})) {
    if (!seen.has(id)) rows.push(toRow(id, { displayName: status.builtinProviders[id] }, !builtinDisabled.has(id), 'builtin', creds[id]))
  }
  return rows
}
function sortRows(rows: ProviderRow[], order?: string[]): ProviderRow[] {
 ...
 */
function sortRowsWithDisabled(rows: ProviderRow[], order?: string[], disabledOrder?: string[]): ProviderRow[] {
  const enabled = rows.filter((r) => r.active)
  const disabled = rows.filter((r) => !r.active)
  const sortedEnabled = sortRows(enabled, order)
  const rankD = new Map<string, number>()
  ;(disabledOrder || disabled.map((r) => r.provider)).forEach((id, i) => rankD.set(id, i))
  const sortedDisabled = [...disabled].sort(
    (a, b) => (rankD.get(b.provider) ?? Number.MAX_SAFE_INTEGER) - (rankD.get(a.provider) ?? Number.MAX_SAFE_INTEGER)
  )
...
function applyOrder<T extends { id?: string }>(items: T[], order: string[]): T[] {
  ...
  const rank = new Map<string, number>()
  order.forEach((id, i) => rank.set(id, i))
  const ranked: T[] = []; const rest: T[] = []
  for (const item of items) {
    const pos = item && typeof item.id === 'string' ? rank.get(item.id) : undefined
    if (pos !== undefined) ranked.push(item); else rest.push(item)
  }
  ranked.sort((a, b) => (rank.get(a.id as string) ?? 0) - (rank.get(b.id as string) ?? 0))
  return [...ranked, ...rest]
}
function isHiddenId(id: unknown): boolean { return typeof id === 'string' && filterState.hidden.has(id) }

// Drag helpers
const ROW_GAP = 8
function computeOffsets(baseOrder: string[], liveOrder: string[], heights: Record<string, number>): Record<string, number> {
  const topBase: Record<string, number> = {}; let y = 0
  for (const p of baseOrder) { topBase[p] = y; y += (heights[p] ?? 0) + ROW_GAP }
  y = 0
  const topLive: Record<string, number> = {}
  for (const p of liveOrder) { topLive[p] = y; y += (heights[p] ?? 0) + ROW_GAP }
...
interface AddModelDraft { uid: number; id: string; name?: string; tiers: string[] }

// ---------------------------------------------------------------------------
// Panel Component
// ---------------------------------------------------------------------------

function BetterModelSettingPanel(props: any): React.ReactElement {
  const { t: userT } = props
  ensureStyles()
  ...
  const [setting, setSetting] = React.useState<BetterModelSetting>({
  ...
  const [documentPath, setDocumentPath] = React.useState<string | undefined>(undefined)
  ...
  const [draftModels, setDraftModels] = React.useState<AddModelDraft[]>([])
  ...
  const load = async () => {
    ...
    try { applyStatus(await getStatus()) } catch (error: any) { setLoadError(error?.message || String(error)) }
  ...
  const flushApply = (next: BetterModelSetting) => {
    ...
    }).finally(() => { if (gen === saveGeneration.current) setSaveBusy(false) })
  }
  const queueApply = (next: BetterModelSetting) => {
    if (saveTimer.current) clearTimeout(saveTimer.current)
    saveTimer.current = setTimeout(() => flushApply(next), 250)
  }
  const updateSetting = (updater: (prev: BetterModelSetting) => BetterModelSetting) => {
    setSetting((prev) => { const next = updater(prev); queueApply(next); return next })
  }
  // 编辑器内的草稿更新：只改 React state，不触发自动保存
  const updateSettingDraft = (updater: (prev: BetterModelSetting) => BetterModelSetting) => {
    setSetting((prev) => { const next = updater(prev); return next })
    setDirty(true)
  }
  const [dirty, setDirty] = React.useState(false)
  const [pendingCloseProvider, setPendingCloseProvider] = React.useState<string | null>(null)
  const closeEditorState = (provider: string) => {
  ...
  const requestCloseEditor = async (provider: string, opts?: { saveFirst?: boolean; discard?: boolean }) => {
    ...
    if (opts?.saveFirst) {
      ...
      if (ok) closeEditorState(provider)
      // 保存失败时保留编辑器与草稿，不关闭
      return
    }
    if (dirty) {
  ...
  const confirmDelete = async () => {
    ...
    setDeleting(true); setDeleteFailure('')
    try {
  ...
  const submitOfficial = async () => {
    setOfficialBusy(true); setOfficialError('')
    try {
      ...
      setSaveMsg(text('savedProvider', '已保存。'))
      setTimeout(() => setSaveMsg(''), 1500)
    } catch (error: any) { setOfficialError(error?.message || String(error)) }
    finally { setOfficialBusy(false) }
  }

  // Quick toggle (no confirm)
  const toggleProvider = (provider: string, kind: string, active: boolean) => {
    if (kind === 'builtin') {
  ...
  const toggleBuiltinEnabled = (provider: string, enabled: boolean) => {
    updateSetting((prev) => {
      const builtinDisabled = new Set(prev.builtinDisabled || [])
      if (enabled) builtinDisabled.delete(provider); else builtinDisabled.add(provider)
  ...
  const submitAdd = async () => {
    ...
    if (!provider) { setAddError(text('modelIdRequired', 'Provider ID 不能为空。')); return }
    if (!/^[a-z][a-z0-9-]*$/.test(provider)) { setAddError(text('customRouteInvalid', '需以小写字母开头，之后可用小写字母、数字和短横线。')); return }
    if (rows.some((r) => r.provider === provider)) { setAddError(text('customRouteTaken', '已有提供方使用了这个 ID。')); return }
    if (!draft.baseURL.trim()) { setAddError(text('customNeedsBaseUrl', '自定义提供方需要填写 API 地址。')); return }
    const validModels = draftModels.filter((m) => m.id.trim().length > 0)
    if (validModels.length === 0) { setAddError(text('customNeedsModels', '自定义提供方至少需要一个模型。')); return }
    ...
    try {
      ...
      profile.models = validModels.map((m) => {
        ...
        if (m.tiers.length > 0) { entry.reasoningEfforts = {}; for (const t of m.tiers) entry.reasoningEfforts[t] = t === 'off' ? null : t }
        return entry
      })
      ...
      setSaveMsg(text('savedProvider', '已保存。'))
      setTimeout(() => setSaveMsg(''), 1500)
    } catch (error: any) { setAddError(error?.message || String(error)) }
    finally { setAddBusy(false) }
  }
  const addModelRow = () => setDraftModels((prev) => [{ uid: ++addModelUid, id: '', name: '', tiers: [] }, ...prev])
  const removeModelRow = (index: number) => setDraftModels((prev) => prev.filter((_, i) => i !== index))
  const updateModelRow = (index: number, patch: Partial<AddModelDraft>) => setDraftModels((prev) => prev.map((m, i) => (i === index ? { ...m, ...patch } : m)))
  const toggleModelTier = (index: number, tier: string) => setDraftModels((prev) => prev.map((m, i) => {
    if (i !== index) return m
    const tiers = new Set(m.tiers)
    if (tiers.has(tier)) tiers.delete(tier); else tiers.add(tier)
    return { ...m, tiers: [...tiers] }
  }))

  // Edit model additions
  const addEditModelRow = (provider: string) => setModelDraft((prev) => ({
    ...prev,
  ...
  const toggleEditModelTier = (provider: string, key: string, tier: string) => setModelDraft((prev) => {
  ...
  const rowElsRef = React.useRef<Record<string, HTMLElement | null>>({})
  const captureHeights = (keys: string[]) => {
    const h: Record<string, number> = {}
    for (const k of keys) { const el = rowElsRef.current[k]; if (el) h[k] = el.offsetHeight }
    heightsRef.current = h
  }
  const onDragStart = (e: any, provider: string) => {
    dragSrc.current = provider; const base = rows.map((r) => r.provider); captureHeights(base); setLiveOrder(base.slice()); setOverKey(provider)
    if (e.dataTransfer) { e.dataTransfer.effectAllowed = 'move'; e.dataTransfer.setData('text/plain', provider) }
  }
  const onDragOverRow = (e: any, target: string) => {
    if (e.preventDefault) e.preventDefault()
    if (e.dataTransfer) e.dataTransfer.dropEffect = 'move'
    const src = dragSrc.current; if (!src) { setOverKey(target); return }
    const base = rows.map((r) => r.provider); const from = base.indexOf(src); const to = base.indexOf(target)
    if (from < 0 || to < 0 || from === to) { setOverKey(target); return }
    const next = base.slice(); next.splice(from, 1); next.splice(to, 0, src); setLiveOrder(next); setOverKey(target)
  }
  const onDrop = () => {
    const src = dragSrc.current; const order = liveOrder
    if (src && order) {
      const byIdx = order.map((id) => rows.find((r) => r.provider === id)).filter((r): r is ProviderRow => r !== undefined)
      if (byIdx.length === rows.length) setRows(byIdx)
      setNoTransition(true); requestAnimationFrame(() => requestAnimationFrame(() => setNoTransition(false)))
      updateSetting((prev) => ({ ...prev, providerOrder: order.slice() }))
    }
    dragSrc.current = null; setLiveOrder(null); setOverKey(null); heightsRef.current = {}
  }
  const onDragEnd = () => { dragSrc.current = null; setLiveOrder(null); setOverKey(null); heightsRef.current = {} }

  // Effort / retry
  const getEffortEntry = (provider: string, modelId: string): ModelEffortSetting => normalizeEffortEntry(setting.modelEfforts[provider]?.[modelId])
  const setEffortTiers = (provider: string, modelId: string, tiers: string[]) => updateSettingDraft((prev) => {
    const modelEfforts = { ...prev.modelEfforts }; const pm = { ...(modelEfforts[provider] || {}) }
    const existing = normalizeEffortEntry(pm[modelId])
    const next: ModelEffortSetting = { tiers: tiers.length > 0 ? tiers : undefined, selected: existing.selected && tiers.includes(existing.selected) ? existing.selected : undefined }
    if (next.tiers === undefined && next.selected === undefined) delete pm[modelId]; else pm[modelId] = next
    modelEfforts[provider] = pm; return { ...prev, modelEfforts }
  })
  const setEffortSelected = (provider: string, modelId: string, selected: string) => updateSettingDraft((prev) => {
    const modelEfforts = { ...prev.modelEfforts }; const pm = { ...(modelEfforts[provider] || {}) }
    const existing = normalizeEffortEntry(pm[modelId])
    const next: ModelEffortSetting = { tiers: existing.tiers, selected: selected || undefined }
    if (next.tiers === undefined && next.selected === undefined) delete pm[modelId]; else pm[modelId] = next
    modelEfforts[provider] = pm; return { ...prev, modelEfforts }
  })
  const getRetry = (provider: string) => setting.providerRetryOverrides[provider]?.maxRetries ?? 2
  const setRetry = (provider: string, value: number) => updateSettingDraft((prev) => ({
    ...prev,
  ...
  const saveSettings = async (provider?: string): Promise<boolean> => {
    ...
    try {
        ...
        if (draftMap && Object.keys(draftMap).length > 0) {
          ...
          for (const [modelId, d] of Object.entries(draftMap) as Array<[string, { id?: string; name: string; context: string; maxTokens: string; tiers?: string[]; input?: string }]>) {
            if (modelId.startsWith(NEW_MODEL_PREFIX)) {
              ...
              if (!newId) continue
              const update: any = { id: newId, oldId: modelId }
              if (d.name?.trim()) update.name = d.name.trim()
              const nctx = parseCapacity(d.context); if (nctx !== undefined && Number.isFinite(nctx) && nctx > 0) update.contextWindow = nctx
              const nmt = parseCapacity(d.maxTokens); if (nmt !== undefined && Number.isFinite(nmt) && nmt > 0) update.maxTokens = nmt
              if (Array.isArray(d.tiers) && d.tiers.length > 0) {
                const efforts: Record<string, any> = {}; for (const tier of d.tiers) efforts[tier] = tier === 'off' ? null : tier; update.reasoningEfforts = efforts
              }
              edits.push(update); continue
            }
            const oldId = modelId; const newId = d.id?.trim() || oldId
            const update: any = { id: newId, oldId }
            if (d.name?.trim()) update.name = d.name.trim(); else update.name = ''
      ...
      setSaveMsg(text('savedProvider', '已保存。'))
      return true
    } catch (error: any) { setSaveMsg('保存失败: ' + (error?.message || String(error)), true); return false }
    finally { setSaveBusy(false) }
  }
  ...
  const renderProviderRow = (row: ProviderRow, index: number): React.ReactElement => {
    ...
    return React.createElement('li', {
      key: provider, ref: (el: HTMLElement | null) => { rowElsRef.current[provider] = el },
      className: rowClass, style: rowStyle,
      onDragOver: (e: any) => onDragOverRow(e, provider), onDrop, onDragEnd,
    }, [
      React.createElement('div', { className: 'bms-rowHead', key: 'head' }, [
        React.createElement('div', { className: 'bms-rowIdentity', key: 'id' }, [
          row.kind !== 'builtin' && canDrag ? React.createElement('span', {
            ...
            draggable: true, onDragStart: (e: any) => onDragStart(e, provider),
          }) : null,
          ...
          return [
            ...
            React.createElement('label', { key: 'retry', className: 'bms-field' }, [
              ...
              React.createElement('input', {
                ...
                onChange: (e: any) => setRetry(provider, Math.max(0, Math.min(20, Number.parseInt(e.target.value, 10) || 0))),
              }),
            ]),
            // 模型设置 (models catalog)
            React.createElement('details', { className: 'bms-customized', key: 'customized' }, [
              React.createElement('summary', { className: 'bms-customizedSummary' }, [
                text('modelSettings'),
                row.models.length > 0 ? React.createElement('span', { key: 'pills', className: 'bms-modelPills' },
                  row.models.map((m: ModelRow) => React.createElement('span', { key: m.id, className: 'bms-modelPill' }, m.name || m.id))
                ) : null,
              ]),
              React.createElement('div', { className: 'bms-customizedBody' }, [
                React.createElement('div', { className: 'bms-modelCatalog' }, [
                  ...
                  React.createElement('div', { className: 'bms-modelList' }, row.models.map((model: ModelRow) => {
                    ...
                    // 思考强度候选始终列全量（ADD_TIER_CANDIDATES），
                    // 让用户能勾选 provider 未声明的档位；已声明的排前面便于识别。
                    const candidates = [...ADD_TIER_CANDIDATES].sort(
                      (a, b) => (available.includes(b) ? 1 : 0) - (available.includes(a) ? 1 : 0)
                    )
                      ...
                      React.createElement('div', { className: 'bms-modelRow' }, [
                        ...
                        React.createElement('label', { className: 'bms-modelField' }, [
                          ...
                          React.createElement('input', {
                            ...
                            onChange: (e: any) => setDraftField('id', e.target.value),
                          }),
                        ]),
                        // Display name
                        React.createElement('label', { className: 'bms-modelField' }, [
                          ...
                          React.createElement('input', {
                            ...
                            onChange: (e: any) => setDraftField('name', e.target.value),
                          }),
                        ]),
                        // Remove button
                        React.createElement('button', {
                      ...
                      React.createElement('div', { className: 'bms-modelAdvanced' }, [
                        React.createElement('label', { className: 'bms-modelField' }, [
                          ...
                          React.createElement('input', {
                            ...
                            onChange: (e: any) => setDraftField('context', e.target.value),
                          }),
                        ]),
                        React.createElement('label', { className: 'bms-modelField' }, [
                        ...
                        React.createElement('label', { className: 'bms-modelField', style: { gridColumn: '1 / -1' } }, [
                          ...
                          React.createElement('div', { className: 'bms-tierBoxes' }, candidates.map((tier: string) => React.createElement('label', { key: tier, className: 'bms-chip' }, [
                            React.createElement('input', {
                              type: 'checkbox', className: 'bms-chipInput',
                              checked: whitelist.includes(tier),
                              disabled: saveBusy || writable === false,
                              onChange: (e: any) => toggleTier(tier, e.target.checked),
                            }),
                      ...
                      React.createElement('div', { className: 'bms-modelRow' }, [
                        React.createElement('label', { className: 'bms-modelField' }, [
                          ...
                          React.createElement('input', {
                            ...
                            onChange: (e: any) => updateEditModelRow(provider, key, { id: e.target.value }),
                          }),
                        ]),
                        React.createElement('label', { className: 'bms-modelField' }, [
                          React.createElement('span', { className: 'bms-modelFieldLabel' }, text('modelName')),
                          React.createElement('input', {
                            ...
                            onChange: (e: any) => updateEditModelRow(provider, key, { name: e.target.value }),
                          }),
                        ]),
                        React.createElement('button', {
                          type: 'button', className: 'bms-iconButton bms-iconButtonDanger',
                      ...
                      React.createElement('div', { className: 'bms-modelAdvanced' }, [
                        React.createElement('label', { className: 'bms-modelField' }, [
                          ...
                          React.createElement('input', {
                            ...
                            onChange: (e: any) => updateEditModelRow(provider, key, { context: e.target.value }),
                          }),
                        ]),
                        React.createElement('label', { className: 'bms-modelField' }, [
                          React.createElement('span', { className: 'bms-modelFieldLabel' }, text('maxTokens')),
                          React.createElement('input', {
                            ...
                            onChange: (e: any) => updateEditModelRow(provider, key, { maxTokens: e.target.value }),
                          }),
                        ]),
                        React.createElement('label', { className: 'bms-modelField', style: { gridColumn: '1 / -1' } }, [
  ...
  return React.createElement('div', { className: 'better-model-setting bms-section' }, [
    ...
    showOfficialForm && !officialAdded ? React.createElement('div', { className: 'bms-addCard', key: 'officialForm' }, [
      ...
      React.createElement('label', { className: 'bms-field' }, [
        ...
        React.createElement('input', {
          ...
          onChange: (e: any) => setOfficialEnvName(e.target.value),
        }),
      ]),
      React.createElement('label', { className: 'bms-field' }, [
        ...
        React.createElement('input', {
          ...
          onChange: (e: any) => setOfficialApiKey(e.target.value),
        }),
      ]),
    ...
    React.createElement('ul', { className: 'bms-rows', key: 'mainRows' },
      ...
      ? React.createElement('div', { className: 'bms-addCard', key: 'addCard' }, [
          ...
          React.createElement('label', { key: 'provider', className: 'bms-field' }, [
            ...
            React.createElement('input', {
              ...
              onChange: (e: any) => setDraft({ ...draft, provider: e.target.value }),
            }),
          ]),
          React.createElement('label', { key: 'displayName', className: 'bms-field' }, [
            ...
            React.createElement('input', {
              ...
              onChange: (e: any) => setDraft({ ...draft, displayName: e.target.value }),
            }),
          ]),
          React.createElement('label', { key: 'api', className: 'bms-field' }, [
            ...
            React.createElement('select', {
              ...
              onChange: (e: any) => setDraft({ ...draft, api: e.target.value }),
            }, [
          ...
          React.createElement('label', { key: 'baseURL', className: 'bms-field' }, [
            ...
            React.createElement('input', {
              ...
              onChange: (e: any) => setDraft({ ...draft, baseURL: e.target.value }),
            }),
          ]),
          React.createElement('label', { key: 'apiKeyEnv', className: 'bms-field' }, [
            ...
            React.createElement('input', {
              ...
              onChange: (e: any) => setDraft({ ...draft, apiKeyEnv: e.target.value }),
            }),
          ]),
                ...
                React.createElement('div', { className: 'bms-modelRow' }, [
                  React.createElement('label', { className: 'bms-modelField' }, [
                    ...
                    React.createElement('input', {
                      ...
                      placeholder: 'gpt-5.6',
                      onChange: (e: any) => updateModelRow(index, { id: e.target.value }),
                    }),
                  ]),
                  React.createElement('label', { className: 'bms-modelField' }, [
                    React.createElement('span', { className: 'bms-modelFieldLabel' }, text('modelName')),
                    React.createElement('input', {
                      ...
                      onChange: (e: any) => updateModelRow(index, { name: e.target.value }),
                    }),
                  ]),
                  React.createElement('button', {
                    type: 'button', className: 'bms-iconButton bms-iconButtonDanger',
    ...
    // 删除确认弹窗
    deleteTarget ? React.createElement('div', { className: 'bms-overlay', key: 'overlay', onClick: () => !deleting && setDeleteTarget(null), onKeyDown: (e: any) => { if (e.key === 'Escape' && !deleting) setDeleteTarget(null) } }, [
    ...
    // 未保存修改确认弹窗
    pendingCloseProvider ? React.createElement('div', { className: 'bms-overlay', key: 'unsavedOverlay', onClick: () => setPendingCloseProvider(null), onKeyDown: (e: any) => { if (e.key === 'Escape') setPendingCloseProvider(null) } }, [
      React.createElement('div', { className: 'bms-dialog', key: 'dialog', onClick: (e: any) => e.stopPropagation() }, [
...
export function apply(ctx: any): void {
  ctx.effect(() => {
    ...
    const filterDirectoryResponse = (response: any) => {
      const targets: any[] = []
      if (response && typeof response === 'object') {
        if (response.result && typeof response.result === 'object') targets.push(response.result)
        if (response.ok !== undefined || response.value !== undefined) targets.push(response)
      }
      for (const r of targets) {
        if (r && r.ok && r.value && typeof r.value === 'object') {
          if (Array.isArray(r.value.groups)) r.value.groups = applyOrder(
            r.value.groups.filter((g: any) => !(g && isHiddenId(g.id))), filterState.order)
          if (Array.isArray(r.value.failures)) r.value.failures = r.value.failures.filter((f: any) => !(f && isHiddenId(f.id)))
        }
      }
    }
    const filterProvidersResponse = (response: any) => {
      const targets: any[] = []
      if (response && typeof response === 'object') {
        if (response.result && typeof response.result === 'object') targets.push(response.result)
        if (response.ok !== undefined || response.value !== undefined) targets.push(response)
      ...
      for (const r of targets) {
        if (r && r.ok && r.value && Array.isArray(r.value.providers)) r.value.providers = r.value.providers.filter((p: any) => !(p && isHiddenId(p.provider)))
      }
    }
    const wrapModels = (face: any, key: string, filter: (response: any) => void) => {
      if (!face || typeof face !== 'object') return
      const orig = face[key]
      if (typeof orig !== 'function' || (orig as any).__bmsWrapped) return
      const wrapped = (request: any, ...rest: any[]) => {
        const result = orig.call(face, request, ...rest)
        return Promise.resolve(result).then(async (response: any) => {
          await bootstrapped; filter(response); return response
        })
      }
      ;(wrapped as any).__bmsWrapped = true
      face[key] = wrapped
    ...
    const wrapFaces = () => {
      const conn = getConnection(); const api = conn && typeof conn === 'object' ? conn.api : undefined
    ...
    const disposeSlots = ctx.slots.inject('settings.section', () =>
      ctx.slots.register({
        ...
        label: () => '模型',
        inject: () => ({
          t: ctx.locale?.bind?.('better-model-setting') ?? ((s: string) => s),
        }),
      }, (props: any) => {
