...
function BetterModelSettingPanel(props) {
    ...
    const [providerSyncTarget, setProviderSyncTarget] = React.useState(null);
    const [providerSyncGroups, setProviderSyncGroups] = React.useState([]);
    const [providerSyncPicked, setProviderSyncPicked] = React.useState({});
    ...
    const [discoverBusy, setDiscoverBusy] = React.useState('');
    ...
    const openProviderSync = async (provider) => {
        ...
        try {
            const result = await postRaw({ op: 'discover', provider, request: {} });
    ...
    const openGlobalSync = async () => {
        ...
        for (const p of providers) {
            try {
                const result = await postRaw({ op: 'discover', provider: p.provider, request: {} });
                const models = Array.isArray(result?.models) ? result.models : [];
    ...
    const submitProviderSync = async () => {
        ...
        setProviderSyncBusy(true);
        try {
            const status = await postRaw({ op: 'addModels', providerAddModels });
        ...
        try {
            ...
            if (provider) {
                ...
                if (deletes && deletes.length > 0)
                    body.providerDeleteModels = { [provider]: deletes };
    ...
    return React.createElement('div', { className: 'better-model-setting bms-section' }, [
        ...
        providerSyncTarget ? React.createElement('div', {
            ...
            onKeyDown: (e) => { if (e.key === 'Escape' && !providerSyncBusy)
                closeProviderSync(); },
        }, [
            React.createElement('div', {
                ...
                React.createElement('h3', { className: 'bms-dialogTitle' }, providerSyncTarget.mode === 'all' ? text('syncUpstreamAllTitle', '同步所有上游模型') : text('syncUpstreamOneTitle', '同步 {provider} 的上游模型').replace('{provider}', providerSyncTarget.provider || '')),
                ...
                React.createElement('div', {
                    ...
                    const picked = providerSyncPicked[g.provider] || new Set();
                    ...
                    return React.createElement('div', { key: g.provider, className: 'bms-addCard', style: { padding: 8 } }, [
                        ...
                        ...g.models.map((m) => React.createElement('label', { key: m.id, className: 'bms-chip', style: { marginRight: 4, marginBottom: 4 } }, [
                            ...
                            React.createElement('span', { className: 'bms-chipText', key: 't' }, m.id + (m.name ? ` (${m.name})` : '') + (typeof m.contextWindow === 'number' ? ` · ${m.contextWindow}` : '')),
                        ])),
                    ]);
                })),
                React.createElement('div', { className: 'bms-dialogActions' }, [
