...
function isLoopbackOrigin(req) {
    ...
    if (header === undefined)
        return true; // non-browser / same-origin fetch
    try {
 ...
 */
function getEffortSelected(settings, provider, model) {
    if (!settings)
        return undefined;
...
function isValidRetryMode(mode) {
    return mode === 'normal' || mode === 'always';
}
function normalizeRetryOverride(value) {
    if (!isPlainObject(value))
        return undefined;
    const override = { ...value };
    if (override.mode !== undefined && !isValidRetryMode(override.mode))
        return undefined;
    ...
    if (override.retryableCodes !== undefined) {
        if (!Array.isArray(override.retryableCodes) || !override.retryableCodes.every((code) => typeof code === 'string' && /^[1-5][0-9]{2}$/.test(code)))
            return undefined;
    }
    if (override.backoff !== undefined) {
        if (!isPlainObject(override.backoff))
            return undefined;
        const backoff = { ...override.backoff };
        for (const key of ['initialDelayMs', 'maxDelayMs', 'jitterRatio']) {
            if (backoff[key] !== undefined && typeof backoff[key] !== 'number')
                return undefined;
        }
...
export function apply(ctx, _config) {
    ...
    const disposeCurrentWatch = scope.watch((next) => {
        current = next;
    });
    ...
    function backupYaml() {
        ...
        try {
            ...
            const files = fs.readdirSync(backupDir)
                .filter((f) => f.startsWith('settings-') && f.endsWith('.yaml'))
                .sort();
    ...
    async function credentialStateOf(profile) {
    ...
    function buildSyncableProviders(enabledProviders) {
        const out = [];
        ...
        for (const [id, profile] of Object.entries(enabledProviders || {})) {
            ...
            if (typeof profile.apiKeyEnv === 'string' && profile.apiKeyEnv.length > 0)
                entry.apiKeyEnv = profile.apiKeyEnv;
            if (typeof profile.api === 'string' && profile.api.length > 0)
                entry.api = profile.api;
    ...
    async function buildStatus() {
        const llmConfig = ctx.settings.get(LLM_NS);
    ...
    async function handleDisable(provider) {
        const llmConfig = ctx.settings.get(LLM_NS);
        ...
        if (profile === undefined)
            throw new Error(`provider "${provider}" is not present in llm-pi-ai`);
        const db = readDb();
        if (db.disabledProviders[provider] !== undefined)
            throw new Error(`provider "${provider}" is already disabled`);
        ...
        if (!disabledOrder.includes(provider))
            disabledOrder.push(provider);
    ...
    async function handleEnable(provider) {
        const db = readDb();
        const profile = db.disabledProviders[provider];
        if (profile === undefined)
            throw new Error(`provider "${provider}" has no saved snapshot`);
        backupYaml();
        try {
        ...
        next.disabledOrder = next.disabledOrder?.filter((id) => id !== provider);
        writeDb(next);
        syncDisabledSet();
        // 恢复 provider 在 providerOrder 中的原位置（OP-08）
        const order = (current.providerOrder || []).filter((id) => id !== provider);
        order.push(provider);
        if (JSON.stringify(order) !== JSON.stringify(current.providerOrder || [])) {
            await scope.replace({ ...current, providerOrder: order });
        }
    }
    async function handleApply(candidate, providerModels, providerProfile, providerIdRename, providerDeleteModels, providerAddModels) {
        ...
        if (providerIdRename && typeof providerIdRename === 'object' &&
            ...
            if (oldProfile === undefined)
                throw new Error(`provider "${providerIdRename.oldId}" not found`);
            if (providers[providerIdRename.newId] !== undefined)
                throw new Error(`provider "${providerIdRename.newId}" already exists`);
            ...
            if (next.modelEfforts?.[providerIdRename.oldId]) {
                const m = { ...next.modelEfforts };
                m[providerIdRename.newId] = m[providerIdRename.oldId];
                delete m[providerIdRename.oldId];
                next.modelEfforts = m;
            }
            if (next.providerRetryOverrides?.[providerIdRename.oldId]) {
                const r = { ...next.providerRetryOverrides };
                r[providerIdRename.newId] = r[providerIdRename.oldId];
                delete r[providerIdRename.oldId];
                next.providerRetryOverrides = r;
            }
            next.providerOrder = (next.providerOrder || []).map((id) => id === providerIdRename.oldId ? providerIdRename.newId : id);
            // 迁移 DB 快照键
            const db = readDb();
            if (db.disabledProviders?.[providerIdRename.oldId]) {
                ...
                db.disabledOrder = db.disabledOrder?.map((id) => id === providerIdRename.oldId ? providerIdRename.newId : id);
                writeDb(db);
                syncDisabledSet();
            }
        ...
        if (providerProfile && typeof providerProfile === 'object' && !Array.isArray(providerProfile)) {
            ...
            if (targetProvider) {
                ...
                if (profile && typeof profile === 'object') {
                    ...
                    if (typeof providerProfile.displayName === 'string' && providerProfile.displayName.length > 0)
                        nextProfile.displayName = providerProfile.displayName;
                    if ('baseURL' in providerProfile) {
                        if (typeof providerProfile.baseURL === 'string' && providerProfile.baseURL.length > 0)
                            nextProfile.baseURL = providerProfile.baseURL;
                        else
                            delete nextProfile.baseURL;
                    }
                    if ('apiKeyEnv' in providerProfile) {
                        if (typeof providerProfile.apiKeyEnv === 'string' && providerProfile.apiKeyEnv.length > 0)
                            nextProfile.apiKeyEnv = providerProfile.apiKeyEnv;
                        else
        ...
        if (providerModels && typeof providerModels === 'object' && !Array.isArray(providerModels)) {
            ...
            for (const rawProvider of Object.keys(providerModels)) {
                ...
                for (const u of updates) {
                    if (!u || typeof u !== 'object' || typeof u.id !== 'string' || u.id.length === 0)
                        continue;
        ...
        if (providerDeleteModels && typeof providerDeleteModels === 'object' && !Array.isArray(providerDeleteModels)) {
            for (const rawProvider of Object.keys(providerDeleteModels)) {
                ...
                if (!profile || typeof profile !== 'object')
                    continue;
                const existing = Array.isArray(profile.models) ? profile.models : [];
                const removeSet = new Set(ids.filter((id) => typeof id === 'string' && id.length > 0));
                if (removeSet.size === 0)
                    continue;
                const kept = existing.filter((m) => !(m && typeof m === 'object' && typeof m.id === 'string' && removeSet.has(m.id)));
            ...
            for (const rawProvider of Object.keys(providerAddModels)) {
                ...
                for (const e of entries) {
                    if (!e || typeof e !== 'object' || typeof e.id !== 'string' || e.id.length === 0)
                        continue;
                    if (byId.has(e.id))
                        continue; // 已存在的不覆盖，保留用户自定义
                    const next = { id: e.id };
                    if (typeof e.name === 'string' && e.name.length > 0)
                        next.name = e.name;
                    if (typeof e.contextWindow === 'number' && Number.isFinite(e.contextWindow) && e.contextWindow > 0)
                        next.contextWindow = e.contextWindow;
                    if (typeof e.maxTokens === 'number' && Number.isFinite(e.maxTokens) && e.maxTokens > 0)
                        next.maxTokens = e.maxTokens;
                    byId.set(e.id, next);
                    added++;
                }
                if (added > 0) {
    ...
    async function handleDiscover(provider, request) {
        ...
        let apiKey;
        if (request && typeof request.apiKey === 'string' && request.apiKey.length > 0)
        ...
        if (!apiKey && BUILTIN_PROVIDERS[provider] !== undefined && provider === 'deepseek-official') {
            ...
            try {
                ...
                if (typeof resolved === 'string' && resolved.length > 0)
                    apiKey = resolved;
                else if (typeof process.env[envName] === 'string' && process.env[envName].length > 0)
                    apiKey = process.env[envName];
            }
            catch { /* same */ }
        }
        const discoverRequest = { provider };
        if (baseURL)
            discoverRequest.baseURL = baseURL;
        if (api)
            discoverRequest.api = api;
        if (apiKey)
            discoverRequest.apiKey = apiKey;
    ...
    async function handleAdd(provider, profile) {
        if (typeof provider !== 'string' || provider.length === 0)
            throw new Error('provider is required');
        ...
        if (disabledSet.has(provider))
            throw new Error(`provider "${provider}" was disabled; enable it instead`);
        ...
        if (typeof profile.displayName === 'string' && profile.displayName.length > 0)
            clean.displayName = profile.displayName;
        if (typeof profile.api === 'string' && profile.api.length > 0)
            clean.api = profile.api;
        if (typeof profile.baseURL === 'string' && profile.baseURL.length > 0)
            clean.baseURL = profile.baseURL;
        if (typeof profile.apiKeyEnv === 'string' && profile.apiKeyEnv.length > 0)
            clean.apiKeyEnv = profile.apiKeyEnv;
        if (Array.isArray(profile.models))
            clean.models = profile.models.filter((m) => m && typeof m === 'object' && !Array.isArray(m));
        if (typeof profile.provider === 'string' && profile.provider.length > 0)
            clean.provider = profile.provider;
        if (typeof profile.extraBody === 'object' && profile.extraBody !== null)
            clean.extraBody = profile.extraBody;
        if (!clean.displayName)
            clean.displayName = provider;
        backupYaml();
    ...
    function builtinRelease(profile) {
        return profile && typeof profile === 'object' && profile.unsafe === true;
    }
    async function handleAddOfficial(apiKey, envName) {
    ...
    async function handleDelete(provider) {
        ...
        if (db.disabledProviders[provider] !== undefined) {
            ...
            db.disabledOrder = db.disabledOrder?.filter((id) => id !== provider);
            writeDb(db);
            syncDisabledSet();
            changed = true;
        }
        ...
        nextSetting.providerOrder = (current.providerOrder || []).filter((id) => id !== provider);
        if (nextSetting.modelEfforts?.[provider]) {
            const m = { ...nextSetting.modelEfforts };
            delete m[provider];
            nextSetting.modelEfforts = m;
        }
        if (nextSetting.providerRetryOverrides?.[provider]) {
            const r = { ...nextSetting.providerRetryOverrides };
            delete r[provider];
            nextSetting.providerRetryOverrides = r;
        }
            ...
            handler: async (req, res) => {
                ...
                if (req.method === 'GET') {
                    ...
                    if (typeof reqToken === 'string' && reqToken.length > 0) {
                        if (reqToken === authToken)
                            return json(res, 200, { ok: true, value: status });
                        return json(res, 403, { ok: false, error: { message: 'forbidden' } });
                    }
                ...
                try {
                    switch (body.op) {
                        ...
                        case 'addOfficial': {
                            await handleAddOfficial(typeof body.apiKey === 'string' ? body.apiKey : undefined, typeof body.envName === 'string' && body.envName.length > 0 ? body.envName : 'DEEPSEEK_API_KEY');
                            return json(res, 200, { ok: true, value: await buildStatus() });
                        }
                        case 'delete': {
                            if (typeof body.provider !== 'string' || !body.provider) {
    ...
    const disposeRequest = ctx.on('agent/request', async (_payload, next) => {
        ...
        const effortEntry = provider && model ? current.modelEfforts?.[provider]?.[model] : undefined;
    ...
    const disposeRetry = ctx.on('agent/request-error', async (payload, next) => {
        ...
        if (!override || Object.keys(override).length === 0)
            return next();
        const merged = deepMerge(payload.retryPolicy, override);
